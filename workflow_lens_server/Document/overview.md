...
│   │   ├── dialect.go       # SQL方言定義
│   │   ├── driver_default.go    # SQLiteドライバ（デフォルト）
│   │   ├── driver_postgres.go   # PostgreSQLドライバ（ビルドタグ）
...
| DBインターフェース | `database/sql`（標準） | Go標準のDB抽象化。ドライバを差し替えるだけでSQLite/PostgreSQL/MySQL等に対応 |
...
| `OTEL_SERVICE_NAME` | いいえ | OTelサービス名 | `workflow_lens_server` |
