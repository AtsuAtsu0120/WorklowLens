...
workflow_lensの多重起動を防止し、同一マシンで常に1インスタンスのみ動作することを保証する。

## 背景・目的

workflow_lensはローカルの中継ツールであり、同じマシンで複数起動する必要がない。誤って2つ目を起動すると、同じポートの取り合いやログの分散が発生する。

## 要件

- [x] 同じマシンで2つ目のworkflow_lensを起動しようとしたとき、分かりやすいエラーメッセージを表示して終了する
