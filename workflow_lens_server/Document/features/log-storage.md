...
- [x] `details` フィールドをDBに保存する（PostgreSQL: JSONB、SQLite: TEXT、MySQL: JSON）
...
| ドライバ | details列 | プレースホルダ | ID列 |
|---------|----------|-------------|------|
| `sqlite` | TEXT | `?` | INTEGER PRIMARY KEY AUTOINCREMENT |
| `postgres` / `pgx` | JSONB | `$N` | BIGINT GENERATED ALWAYS AS IDENTITY |
| `mysql` | JSON | `?` | BIGINT AUTO_INCREMENT |
