...
internal/model/log_message.go → LogMessage、カテゴリ定義（別仕様: log-message.md）
...
- [x] 空データグラムを無視する
- [x] 複数クライアントからの送信
- [x] オプションフィールド（user_id, duration_ms）を含むメッセージの送受信
- [x] セッションライフサイクル（session/start → edit/xxx → session/end）
